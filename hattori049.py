...
class macaAI(OthelloAI):
    ...
    def move(self, board, color: int) -> tuple[int, int]:
        ...
        # 評価関数に基づいて各升目の評価値を計算
        eval_values = [[0 for _ in range(8)] for _ in range(8)]
        for i in range(8):
            for j in range(8):
                if (i, j) in valid_moves:
                    eval_values[i][j] = self.eval_weights[i][j]

        # 最も評価値が高い升目を選ぶ
        max_value = max(max(row) for row in eval_values)
        best_moves = [(i, j) for i in range(8) for j in range(8) if eval_values[i][j] == max_value]
