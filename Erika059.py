...
class erkOthello(OthelloAI):
    ...
    def move(self, board: np.array, piece: int) -> tuple[int, int]:
        ...
        while True:
            try:
                print(f"{self.face}{self.name}, choose your move (row, col): ")
                ...
                print("Invalid input format. Please enter row and col as integers separated by a comma.")
...
class MinMaxAI(OthelloAI):
    ...
    def move(self, board: np.array, piece: int) -> tuple[int, int]:
        _, best_move = self.minmax(board, piece, self.depth)
        return best_move

    def minmax(self, board: np.array, piece: int, depth: int) -> tuple[int, tuple[int, int]]:
        if depth == 0 or len(get_valid_moves(board, piece)) == 0:
            # 評価関数を用いて現在のボードの評価値を返す
            return self.evaluate_board(board, piece), None

        valid_moves = get_valid_moves(board, piece)
        if piece == BLACK:  # 自分の手番
            best_value = float('-inf')
            best_move = None
            for move in valid_moves:
                new_board = board.copy()
                flip_stones(new_board, move[0], move[1], piece)
                value, _ = self.minmax(new_board, -piece, depth - 1)
                if value > best_value:
                    best_value = value
                    best_move = move
    ...
    def evaluate_board(self, board: np.array, piece: int) -> int:
        # 現在のボードの評価値を計算する（簡単な評価関数の例）
