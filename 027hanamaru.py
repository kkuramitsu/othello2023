...
class hanamaruAI(OthelloAI):
    ...
    def move(self, board, color: int) -> tuple[int, int]:
        ...
        # 四つ角を取る手があれば、その中からランダムに選ぶ
        corner_moves = [(r, c) for r, c in valid_moves if (r, c) in [(0, 0), (0, len(board)-1), (len(board)-1, 0), (len(board)-1, len(board)-1)]]
        if corner_moves:
            return random.choice(corner_moves)

        # 四つ角以外の手があれば、その中からランダムに選ぶ
        if valid_moves:
            return random.choice(valid_moves)

        # 手がなければ何もせずに返す
        return (0, 0)  # 例として (0, 0) を返していますが、ここを適切な処理に変えてください
